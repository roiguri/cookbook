...
`initLogger()` is invoked at app start in `src/app.js`. It schedules the dynamic `import('@sentry/browser')` via `requestIdleCallback` (5 s timeout; `setTimeout(load, 3000)` fallback for Safari < 16.4) so the SDK's chunk fetch, parse, and `Sentry.init()` all run during browser idle — see [Performance impact](#performance-impact). Capture calls fired before the SDK lands are queued in-module and replayed once init resolves; if init resolves inactive (no DSN / dev), the queue is dropped.
...
|                       | Cost                                      | Notes                                                                                                                                                                           |
| --------------------- | ----------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Entry bundle          | 0                                         | SDK is dynamically imported, lands in its own chunk (~450 KB raw / ~149 KB gz). The import is wrapped in `requestIdleCallback` so the fetch + parse + `init()` all run on idle. |
| `Sentry.init()`       | ~5–15 ms                                  | One-time, runs during browser idle. Pre-init errors are queued and replayed, not lost.                                                                                          |
| `captureError()` call | <1 ms sync + async send                   | JSON serialize + fire-and-forget POST.                                                                                                                                          |
...
The loader pattern (dynamic import + idle defer + queue) is the canonical answer for any non-UI SDK; rationale and failure modes are in [`docs/lessons/performance.md`](../lessons/performance.md).
