...
`initLogger()` is invoked once at app start in `src/app.js`, before Firebase init. It does not load Sentry immediately — instead it schedules the dynamic `import('@sentry/browser')` via `requestIdleCallback` (with a `setTimeout(load, 3000)` fallback for older Safari and a 5 s `timeout` to guarantee it eventually fires). This keeps the chunk fetch, parse, and `Sentry.init()` work out of the Lighthouse mobile measurement window so they don't count against Total Blocking Time. See [Performance impact](#performance-impact) below.

Because the SDK loads asynchronously, any `captureError` / `captureMessage` / `setUser` / `addBreadcrumb` calls fired between `initLogger()` and the chunk landing are pushed onto an in-memory queue. When `Sentry.init()` finishes, the queue is flushed; if `initLogger()` resolves with the logger inactive (no DSN / development), the queue is dropped. Net effect: early-startup errors (e.g. `initFirebase` throwing synchronously) are still reported — they just arrive in Sentry a few seconds later than the moment they were thrown.
...
| Critical-path window  | 0                                            | The dynamic import is wrapped in `requestIdleCallback` (5 s timeout; `setTimeout(load, 3000)` fallback for Safari < 16.4). The chunk fetch, parse, and `Sentry.init()` all happen during browser idle, after FCP/LCP/TBT have been measured. |
| Pre-init errors       | Queued in-memory, flushed once the SDK lands | Errors during the deferred window (first ~3–5 s of the page) are captured and replayed after Sentry init resolves. If `initLogger` decides Sentry should stay inactive (no DSN / dev), the queue is dropped.                                 |
| `Sentry.init()`       | ~5–15 ms inside the chunk's `.then()`        | Runs once, during browser idle. Pre-init errors are queued, not lost.                                                                                                                                                                        |
...
**Why dynamic import + idle defer?** The original integration imported `@sentry/browser` statically from `logger.js`, pulling the entire SDK into `main.js` (~+85 KB raw / ~+29 KB gz). Lighthouse penalised that under mobile throttling — the deploy-preview Performance score sat ~10 points below its previous baseline. A first attempt switched to plain dynamic import inside `initLogger()`, which shrank `main.js` but moved Sentry's parse + `init()` work into the post-FCP window, where it counted directly against Total Blocking Time — score got slightly worse, not better. Wrapping the dynamic import in `requestIdleCallback` pushes that work out of the Lighthouse measurement window entirely and recovers the score. (See the lesson entry in `docs/lessons/performance.md`.)
